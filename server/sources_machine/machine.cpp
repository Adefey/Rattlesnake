...
Machine::Machine(const std::string &start_param_scheme, const std::string &start_param_vars) {
  user = new User();
...
std::string Machine::ProcessAllBlocks() {
  ...
  {
    std::string start_string = MakeProcessStartString(user.block_scheme[i].name);
