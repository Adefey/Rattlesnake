        ...
        for (size_t j = 0; j < ((*blocks)[i])->given_vars.size(); ++j) {
            ...
            QTableWidgetItem* tableItem1 =
                new QTableWidgetItem(QString::fromStdString(
                    ((*blocks)[i])->name +
                    "::" + ((*blocks)[i])->given_vars[j].param_name));
                ...
                for (size_t w = 0; w < ((*blocks)[k])->solved_vars.size(); ++w)
                    comboBox->insertItem(
                        ...
                        QString::fromStdString(
                            ((*blocks)[k])->name +
                            "::" + ((*blocks)[k])->solved_vars[w].param_name));
            }
...
void RequiredVariables::updateAnswers(const std::vector<Parameter>& ans) {
    ...
    for (size_t i = 0; i < ans.size(); ++i) {
        ...
        QTableWidgetItem* tableItem1 =
            new QTableWidgetItem(QString::fromStdString(ans[i].param_name));
        QTableWidgetItem* tableItem2 =
            new QTableWidgetItem(QString::fromStdString(ans[i].param_value));
...
void RequiredVariables::fixExtension(std::string& filename) {
    ...
    if (filename.length() >= ending.length()) {
        if (0 == filename.compare(filename.length() - ending.length(),
...
void RequiredVariables::parseData(std::string& data, std::string& tableData,
...
void RequiredVariables::open(std::string& data) {
    ...
    for (size_t i = 0; i < tableParams.size(); ++i) {
        ...
        combBox->setCurrentText(
            QString::fromStdString(tableParams[i].param_value));
    }
