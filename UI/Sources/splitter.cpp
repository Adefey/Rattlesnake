    ...
    : QWidget(parent)
{
    ...
    QObject::connect(sch, SIGNAL(saving(std::string&)), reqv, SLOT(save(std::string&)));
    QObject::connect(this, SIGNAL(openscheme(std::string&, std::string&)), sch, SLOT(open(std::string&, std::string&)));
    QObject::connect(sch, SIGNAL(openvars(std::string&)), reqv, SLOT(open(std::string&)));
    ...
    if (fileName.isEmpty())
        ...
        if (data.isEmpty()) {
            QMessageBox::information(this, tr("Project is empty"),
                tr("The file you are attempting to open contains no information."));
        } else {
...
void Splitter::parseOpenFile(std::string& data, std::string& datascheme, std::string& datareqv) {
    size_t i = 0;
    while (data[i] != '#') {
        datascheme += data[i++];
    }
